...
--
-- Host: 127.0.0.1
-- Czas generowania: 01 Paź 2018, 22:20
...
--
ALTER TABLE `uczniowie`
  MODIFY `id_ucznia` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=73;
...
SELECT nazwisko, imie, datediff(now(),data_zatr) AS dni FROM personel ORDER BY dni DESC;
SELECT nazwisko, imie, pensja, pensja*3.96 AS zloty, pensja*0.87 AS euro FROM personel WHERE dzial LIKE 'ZBYT';
SELECT sum(cena*il_na_stan) FROM towary;
SELECT count(pensja) AS liczba, sum(pensja) AS suma, max(pensja) AS maksymalna, min(pensja) AS minimalna, avg(pensja) AS srednia FROM personel;
SELECT stan, count(pensja) AS liczba, avg(pensja) AS srednia FROM personel GROUP BY stan HAVING liczba>1;



SELECT nazwisko, imie, klasy.nazwa_klasy FROM uczniowie INNER JOIN klasy ON uczniowie.id_klasy = klasy.id_klasy WHERE klasy.nazwa_klasy = '3ct';
SELECT nazwisko, imie, klasy.nazwa_klasy FROM uczniowie LEFT JOIN klasy ON uczniowie.id_klasy = klasy.id_klasy;